// Shared plumbing for the impossible-world MCP server: session id resolution
// and telemetry events describing the server's own tool calls / resource
// reads. Kept separate from tool logic so each tool file stays focused on
// its actual behavior.
import { createEvent } from "../shared/events.js";
import { recordAndNotify } from "../shared/emit.js";
import { getCurrentSessionId } from "../server/session.js";
 ...
 * fixed pseudo-session when no AGENTARIUM session is currently active, so
