...
const DOMAINS = ["relationships", "changes", "issues", "structures", "institutions"] as const;
...
export function registerSimulateChange(server: McpServer): void {
  server.registerTool(
    "simulate_change",
    {
      ...
      description:
        "Dry-run a shallow patch merged into one domain of the world (relationships, changes, issues, structures, or institutions) and report whether the resulting world would pass validation. Never writes to disk.",
