    ...
    <div className="rounded-lg border border-border bg-surface-raised p-4 shadow-raised sm:p-5">
      ...
      <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-xs sm:grid-cols-3 md:grid-cols-6">
        <div>
          ...
          <dd className="mt-0.5 text-text">{run.trigger}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-wide text-text-muted">Branch</dt>
          <dd className="mono mt-0.5 text-text">{run.branch ?? 'Unavailable'}</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="mt-0.5 text-text">{run.engine ?? 'Unavailable'}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-wide text-text-muted">Started</dt>
          <dd className="mt-0.5 text-text">{formatRelativeTime(run.startTime)}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-wide text-text-muted">Duration</dt>
...
function TimelineTab(): JSX.Element {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-border bg-surface-raised shadow-raised">
      {events?.map((e) => (
        <div
          key={e.id}
          className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-border px-3 py-2 text-xs last:border-b-0"
        >
          <span className="mono w-20 shrink-0 text-text-faint">
      ...
      {events?.length === 0 && <div className="p-4 text-text-muted">No events recorded.</div>}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-2">
      {usages?.map((u) => (
        ...
        >
          <div className="flex items-center justify-between">
            <span className="font-medium text-text">{u.skillDefinition.name}</span>
            <TriStatePill label="Available" value={u.available ? 'true' : 'false'} />
          </div>
          <dl className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div>
              <dt className="text-[10px] uppercase tracking-wide text-text-muted">Configured</dt>
              <dd className="mt-1">
                <TriStatePill value={u.configured ? 'true' : 'false'} />
              </dd>
            </div>
            <div>
              <dt className="text-[10px] uppercase tracking-wide text-text-muted">Loaded</dt>
              <dd className="mt-1">
                <TriStatePill value={u.loaded} />
              </dd>
            </div>
            <div>
              <dt className="text-[10px] uppercase tracking-wide text-text-muted">Referenced</dt>
              <dd className="mt-1">
                <TriStatePill value={u.referenced} />
              </dd>
            </div>
            <div>
              ...
              <dd className="mt-1">
                <TriStatePill value={u.executionEvidence} />
              </dd>
            </div>
          </dl>
        </div>
  ...
  return (
    <div className="flex flex-col gap-2">
      {tools?.map((t) => (
        ...
        >
          <div className="flex items-center justify-between">
            <span className="mono font-medium text-text">{t.toolName}</span>
...
function LogsTab(): JSX.Element {
  ...
  return (
    <div className="mono flex flex-col gap-0.5 rounded-lg border border-border bg-surface-sunken p-3 text-xs leading-relaxed shadow-raised">
      {logs?.map((l) => (
        <div
          key={l.id}
          className={clsx(
    ...
    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
      {[
        ...
        return (
          <div
            key={label as string}
            className="rounded-lg border border-border bg-surface-raised p-3 shadow-raised"
          >
            ...
            <div
              className={clsx(
                ...
                isUnavailable ? 'text-text-faint' : 'text-text',
              )}
            >
              {value ?? 'Unavailable'}
            </div>
        ...
        <div className="flex flex-col gap-1.5">
          {data?.pullRequests.map((pr) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="block rounded-lg border border-border bg-surface-raised p-2.5 text-accent shadow-raised hover:underline"
            >
              #{pr.number} {pr.title}
            </a>
          ))}
      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          {data?.checks.map((c) => (
            <div
              key={c.id}
              className="flex items-center justify-between rounded-lg border border-border bg-surface-raised p-2.5 shadow-raised"
            >
...
function DriftTab(): JSX.Element {
  ...
  if (!findings || findings.length === 0) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-status-success/25 bg-status-success-wash p-3 text-status-success">
  ...
  return (
    <div className="flex flex-col gap-2">
      {findings.map((f) => (
        <div
          key={f.id}
          className="rounded-lg border border-status-running/30 bg-status-running-wash p-3 text-xs shadow-raised"
        >
          <div className="mono flex items-center gap-1.5 font-semibold text-status-running">
          ...
          {(f.expected || f.observed) && (
            <div className="mono mt-2 grid grid-cols-2 gap-2 rounded bg-surface-raised p-2 text-text-muted">
              ...
              <div>
                <div className="text-[10px] uppercase tracking-wide">Observed</div>
