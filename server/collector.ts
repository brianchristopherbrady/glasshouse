...
import { z } from "zod";
import { AgentariumEventSchema, type AgentariumEvent } from "../shared/events.js";
...
const IngestBodySchema = z.union([
  z.object({ events: z.array(AgentariumEventSchema) }),
  AgentariumEventSchema,
]);

/** Shared ingestion path used by both the HTTP route and the MCP server. */
export async function ingestEvent(event: AgentariumEvent): Promise<AgentariumEvent> {
