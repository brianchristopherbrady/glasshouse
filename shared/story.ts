...
export type StoryBeatKind = "root" | "agent" | "subagent" | "decision" | "validation" | "milestone";
...
export interface StoryBeat {
  ...
  character?: string;
  timestamp: string;
  evidence: AgentariumEvent["evidence"];
  reason?: string;
  next?: string;
  alternatives?: string[];
  confidence?: number;
  sourceEvent?: AgentariumEvent;
  parentId: string | null;
  childIds: string[];
  effects: AgentariumEvent[];
  unresolved: boolean;
...
function strMeta(event: AgentariumEvent, key: string): string | undefined {
  const v = event.metadata?.[key];
  return typeof v === "string" ? v : undefined;
}
function numMeta(event: AgentariumEvent, key: string): number | undefined {
  const v = event.metadata?.[key];
  return typeof v === "number" ? v : undefined;
}
function arrMeta(event: AgentariumEvent, key: string): string[] | undefined {
  const v = event.metadata?.[key];
...
function makeBeat(id: string, kind: StoryBeatKind, title: string, event: AgentariumEvent, parentId: string): StoryBeat {
  return {
    id,
    kind,
    title,
 ...
 */
export function buildStoryGraph(events: AgentariumEvent[]): StoryGraph {
