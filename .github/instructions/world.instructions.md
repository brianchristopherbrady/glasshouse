...
(`shared/world-types.ts`): relationships, corrections, anomalies, characters
...
2. Know which file you're changing and why: `relationships.json` (the graph),
   `corrections.json` (changes made to it), `anomalies.json` (open
   incidents), `characters.json` (individual structures), `institutions.json`
...
- A character's `provenanceStatus` should only be `"closed"` if its
  ...
  validator quiet.
- An anomaly classified `systemic_candidate` should document a
