...
(`shared/world-types.ts`): relationships, changes, issues, structures
...
2. Know which file you're changing and why: `relationships.json` (the graph),
   `changes.json` (changes made to it), `issues.json` (open
   issues), `structures.json` (individual structures), `institutions.json`
...
3. Check `.github/instructions/municipal-code.instructions.md` for the real
   ...
   an agent proposing or performing a change should be able to cite the
   relevant section.

## Rules

- Every relationship's `subject`/`object` must be an id that exists in
  `structures.json` or `institutions.json` -- never invent an id in
  `relationships.json` alone.
- A change that severs, redirects, or otherwise removes a relationship
...
- Every change needs an `authorizationId` that some institution actually
  has on file, for an operation that institution is permitted to authorize.
- A structure's `provenanceStatus` should only be `"closed"` if its
