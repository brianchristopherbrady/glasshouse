...
// trace_decision -- these are self-reported facts, not something
// flowbook inferred. Without this tool, shared/story.ts's
// flowchart (Story mode) can only ever populate agent/milestone nodes from
// prerecorded demo traces, because the live hook pipeline
// (scripts/flowbook-hook.mjs) has no event
...
export function registerRecordStoryBeat(server: McpServer): void {
  server.registerTool(
    ...
    async (args: {
      ...
      const metadata = { ...(args.reason !== undefined && { reason: args.reason }), ...(runId !== undefined && { runId }) };
