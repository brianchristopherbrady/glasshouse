...
describe("computeSessionMetrics", () => {
  ...
  it("counts distinct agents, files, and tools", () => {
    const events = [
      ...
      at("2026-01-01T00:00:03.000Z", {
        type: "file.written",
        source: "filesystem",
        metadata: { path: "world/changes.json" },
      }),
    ...
    expect(metrics.filesWritten).toEqual(["world/changes.json"]);
  ...
  it("counts agent handoffs and includes both agents in agentsUsed", () => {
    const events = [
      at("2026-01-01T00:00:00.000Z", {
        type: "agent.handoff",
        evidence: "declared",
    ...
    expect(metrics.agentsUsed).toEqual(["implementer", "planner"]);
  });
});
