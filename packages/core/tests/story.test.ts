...
describe("buildStoryGraph", () => {
  ...
  it("forms a real chain when one agent hands off to another: successor's agent.started parents under the handoff beat", () => {
    const events = [
      at("2026-01-01T00:00:00.000Z", { type: "agent.started", actor: { id: "planner", kind: "agent", name: "planner" } }),
      at("2026-01-01T00:00:01.000Z", {
        type: "agent.handoff",
        evidence: "declared",
        actor: { id: "planner", kind: "agent", name: "planner" },
        label: "planner → implementer",
        metadata: { fromAgent: "planner", toAgent: "implementer", reason: "plan is ready" },
      }),
      at("2026-01-01T00:00:02.000Z", { type: "agent.started", actor: { id: "implementer", kind: "agent", name: "implementer" } }),
    ];
    const graph = buildStoryGraph(events);
    const plannerBeat = [...graph.beats.values()].find((b) => b.character === "planner")!;
    const handoffBeat = [...graph.beats.values()].find((b) => b.kind === "handoff")!;
    const implementerBeat = [...graph.beats.values()].find((b) => b.character === "implementer")!;

    expect(handoffBeat.parentId).toBe(plannerBeat.id);
    expect(handoffBeat.fromAgent).toBe("planner");
    expect(handoffBeat.toAgent).toBe("implementer");
    expect(implementerBeat.parentId).toBe(handoffBeat.id);
    expect(handoffBeat.unresolved).toBe(false);
  });

  it("marks a handoff with no observed follow-through as unresolved", () => {
    const events = [
      at("2026-01-01T00:00:00.000Z", { type: "agent.started", actor: { id: "planner", kind: "agent", name: "planner" } }),
      at("2026-01-01T00:00:01.000Z", {
        type: "agent.handoff",
    ...
    const graph = buildStoryGraph(events);
    const handoffBeat = [...graph.beats.values()].find((b) => b.kind === "handoff")!;
    expect(handoffBeat.unresolved).toBe(true);
    expect(handoffBeat.unresolvedReason).toContain("implementer");
  });
});
