...
function metadataString(event: FlowbookEvent, key: string): string | undefined {
  const value = event.metadata?.[key];
  return typeof value === "string" ? value : undefined;
}

export function computeSessionMetrics(events: FlowbookEvent[]): SessionMetrics {
  ...
  for (const event of sorted) {
    switch (event.type) {
      ...
      case "agent.handoff": {
        handoffCount += 1;
        const from = metadataString(event, "fromAgent");
        const to = metadataString(event, "toAgent");
        if (from) agentsUsed.add(from);
        if (to) agentsUsed.add(to);
        break;
      }
      case "file.read": {
