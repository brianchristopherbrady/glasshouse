...
import { narrate, narrateShort } from "../../shared/narrate.js";

type NodeKind = "root" | "agent" | "subagent" | "tool" | "skill" | "mcp" | "file" | "decision" | "validation";
...
function laneKey(event: AgentariumEvent): string {
  if (event.actor?.kind === "subagent") return `actor:subagent:${event.actor.id}`;
  return "main";
}

function toolFilePath(event: AgentariumEvent): string | undefined {
...
function eventSignature(event: AgentariumEvent): string | null {
  switch (event.type) {
    case "tool.requested":
    case "tool.completed":
    case "tool.failed": {
...
function nodeKindFor(event: AgentariumEvent): NodeKind {
  switch (event.type) {
    case "tool.requested":
    case "tool.completed":
    case "tool.failed":
...
function nodeLabelFor(event: AgentariumEvent): string {
  switch (event.type) {
    case "tool.requested":
    case "tool.completed":
    case "tool.failed": {
...
function buildGraph(events: AgentariumEvent[]): {
  ...
  function touchNode(id: string, label: string, kind: NodeKind, evidence: AgentariumEvent["evidence"], event: AgentariumEvent) {
...
export function EventGraph({
  ...
  events: AgentariumEvent[];
  onSelectEvent?: (event: AgentariumEvent) => void;
}) {
